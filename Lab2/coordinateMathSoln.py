...
def main():
    # Takes the coordinates of C N and Ca
    line = input("Whatis your triad?")
    newLine = line.replace('(', ',').replace(')', ',')
    parts = newLine.split(',')
    pieces = (float(parts[1]), float(parts[2]), float(parts[3]))
    pieces2 = (float(parts[5]), float(parts[6]), float(parts[7]))
    pieces3 = (float(parts[9]), float(parts[10]), float(parts[11]))
    newTriad = Triad(pieces, pieces2, pieces3)
    nCLength = newTriad.dPQ()  # finds N-C bond length
    nCaLength = newTriad.dQR()  # finds N-Ca bond length
    nCCaAngle = (newTriad.angleQ()) * (180.0 / math.pi)  # finds C-N-Ca bond angle
    print("N-C bond length = {0:0.2f} \nN-Ca bond length = {1:0.2f} \nC-N-Ca bond angle = {2:0.1f}".format(nCLength,
                                                                                                           nCaLength,
                                                                                                           nCCaAngle))
